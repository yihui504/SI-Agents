...
| **AOT 真代码生成（US-016）** | SkVM 整 skill → 代码 | code-review skill 9 steps → 3 确定性（直接工具调用）+ 6 判断型（LLM），确定性 33% | codegen ✅，runtime defer |
...
**诚实标注**：
- JIT-optimize 的 0.362→1.000 是**文件层 metric**（toolCalls/sections/risk），与 SkVM 论文的 task-level metric（任务完成质量）不可直接比——SI-agents 这条待 task-level 化
- 代码固化 50x+ 是**保守下限**（rhs <100ms，精确 ms 受测量精度限制；lhs 是完整 LLM 循环成功 run）
